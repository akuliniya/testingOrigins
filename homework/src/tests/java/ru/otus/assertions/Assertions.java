...
public class Assertions {
    ...
    public static void assertInRange(int minExpected, int maxExpected, int actual) {
        if ((maxExpected < actual) || (minExpected > actual)) {
            throw new AssertionError(String.format("Expected from %d to %d, get %d", minExpected, maxExpected, actual));
        }
    }

    public static void assertInRange(long minExpected, long maxExpected, long actual) {
        if (maxExpected < actual || minExpected > actual) {
            throw new AssertionError(String.format("Expected from %d to %d, get %d", minExpected, maxExpected, actual));
        }
    }
